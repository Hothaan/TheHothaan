...
const saveImageFromURL = async (req, res) => {
  ...
  try {
    ...
    // .templateImage 요소가 있는지 확인
    const element = await page.$(".templateImage");
    if (!element) {
      console.error(".templateImage not found");
      await browser.close();
      return res.status(500).json({ message: "Element not found" });
    }
    ...
    const imageDir =
      process.env.IMAGE_DIRECTORY || path.join(__dirname, "../images");
