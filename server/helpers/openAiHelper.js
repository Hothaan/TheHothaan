...
// async function generateOpenAiText(service, serviceTitle, serviceDesc, depth1, depth2, component, structure, content, cnt) {
//   try {
//     let userPrompt;

//     if (depth2 === '상품 목록' && cnt) {
//       // 상품 목록용 프롬프트: cnt 개수만큼 카테고리 목록 생성
//       userPrompt = `Generate ${cnt} categories for the ${depth1} menu ${depth2} page on the '${component}' component of a ${service} website named ${serviceTitle}. Provide the response strictly in JSON format as follows:

//       {
//         "menu": "${depth1}",
//         "feature": "${depth2}",
//         "content": {
//           "categories": ["카테고리1", "카테고리2", ..., "카테고리${cnt}"]
//         }
//       }
...
//       userPrompt = `Please generate content for the '${depth1}' menu and '${depth2}' page in the '${component}' component of a '${service}' website titled '${serviceTitle}'. Follow this structure and use the provided example for reference:
...
//     if (depth2 === '상품 목록' && Array.isArray(responseData.content.categories)) {
//       isValidResponse = responseData.content.categories.length === cnt;
//     } else if (responseData.menu && responseData.feature && responseData.content && responseData.content.title && responseData.content.desc) {
...
async function generateOpenAiText(service, serviceTitle, serviceDesc, depth1, depth2, component, structure, content, cnt) {
  try {
    ...
    if (depth2 === '상품 목록' && cnt) {
      // 상품 목록용 프롬프트: cnt 개수만큼 카테고리 목록 생성
      userPrompt = `Generate ${cnt} categories for the ${depth1} menu ${depth2} page on the '${component}' component of a ${service} website named ${serviceTitle}. 
        This website's purpose is: ${serviceDesc}. 
    ...
    if (structure === null || structure === '' && responseData.content === 'none') {
      isValidResponse = true;
    } else if (depth2 === '상품 목록' && Array.isArray(responseData.content.categories)) {
      isValidResponse = responseData.content.categories.length === cnt;
    } else if (structure === '{title: string;}' && responseData.content && typeof responseData.content.title === 'string') {
      isValidResponse = true;
    } else if (responseData.menu && responseData.feature && responseData.content && responseData.content.title && responseData.content.desc) {
      isValidResponse = true;
    }
